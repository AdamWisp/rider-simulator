...
def main():
    ...
    if st.sidebar.button("Start Simulation", key="run_sim"):
        ...
        st.write(
            f"Instructor done at {inst_finish:.2f} min; "
            f"EXP done at {exp_finish:.2f} min; "
            f"FOC done at {foc_finish:.2f} min"
        )
        ...
        html = (
            ...
            "    if(e<r.lap){ let f=e/r.lap; if(f<1/3) x=regions[1].x+f*3*regions[1].w; "
            "else if(f<2/3) x=regions[2].x+(f-1/3)*3*regions[2].w; "
            "else x=regions[3].x+(f-2/3)*3*regions[3].w; }\n"
