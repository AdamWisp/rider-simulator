...
class RiderTrackSimulation:
    def __init__(self, params: dict):
        ...
        self.inst_done = self.env.event()     # for triggering EXP
        self.exp_done = self.env.event()      # for triggering FOC
        if self.p["nEXP"] == 0:
            self.exp_done.succeed()  # skip EXP if zero

        # Completion barrier
        self.done_event = self.env.event()
        self.total = 1 + self.p["nEXP"] + self.p["nFOC"]
        self.finished = 0
    ...
    def rider(self, rid, rtype):
        ...
        # Enter Gate + Instructor demonstration
        with self.zone_a.request() as req:
            ...
            if rtype == "INST":
                # Instructor has now finished EnterGate; but we want entire track -> delay inst_done until Exit
                pass
        ...
        # Trigger inst_done only after instructor *fully* exits
        if rtype == "INST" and not self.inst_done.triggered:
            self.inst_done.succeed()

        # Trigger exp_done only after *all* EXP exit
        if rtype == "EXP":
            done_count = sum(
                1 for e in self.events
                if e["rider_type"] == "EXP" and e["zone"] == "Exit" and e["action"] == "exit"
            )
            if done_count == self.p["nEXP"] and not self.exp_done.triggered:
                self.exp_done.succeed()

        # Bookkeeping to end simulation
        self.finished += 1
        if self.finished == self.total and not self.done_event.triggered:
...
def create_simplified_gantt(events):
    # Only show Zones A, B, C per rider
    ...
    if not tasks:
        return None
    tdf = pd.DataFrame(tasks).sort_values(["Type", "Task", "Start"])
    colors = {"ZoneA": "#FFA07A", "ZoneB": "#20B2AA", "ZoneC": "#9370DB"}
    fig = ff.create_gantt(
        tdf, colors=colors, index_col="Resource",
        group_tasks=True, show_colorbar=True, title="Zones A→B→C Timeline"
    )
    fig.update_layout(
...
def main():
    ...
    text = st.sidebar.text_area("Scenario JSON", json.dumps(p, indent=2), height=150)
    if st.sidebar.button("Load"):
        try:
            st.session_state.params = json.loads(text)
    ...
    if "results" in st.session_state:
        ...
        st.info("Set parameters and run the simulation.")
